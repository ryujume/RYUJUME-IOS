...
class DetailRyujumeVC: UIViewController, DetailRyujumeProvider {
    ...
    @IBAction func likeBtnAction(_ sender: UIButton) {
        if likeBtn.isSelected == true {
            likeBtn.isSelected = false
            postLikePress(LikeModel(ryujumeId: ryujumeId, isLiked: false))
        } else {
            likeBtn.isSelected = true
            postLikePress(LikeModel(ryujumeId: ryujumeId, isLiked: true))
        }
    }
    ...
    func postLikePress(_ like: LikeModel) {
        guard let param = try? JSONEncoder().encode(like), let params = param.toDictionary() else { return }
        
        httpClient
            .post(url: MainAPI.postLikePress.getPath(), params: params, header: Header.tokenIsExist.getHeader())
            .responseJSON(completionHandler: { [weak self] (response) in
                guard let strongSelf = self else { return }
                ...
                case 200 :
                    DispatchQueue.main.async {
                        strongSelf.setLikeBtnImg()
                    }
                    return
                default :
                    DispatchQueue.main.async {
                        strongSelf.showToast(message: "추천 실패")
                    }
                    print(response.response?.statusCode as Any)
                    return
                }
    ...
    func setLikeBtnImg() {
        if likeBtn.isSelected == true {
            likeBtn.setImage(UIImage(contentsOfFile: "selectedLike"), for: .normal)
        } else {
            likeBtn.setImage(UIImage(contentsOfFile: "selectLike"), for: .normal)
        }
    }
