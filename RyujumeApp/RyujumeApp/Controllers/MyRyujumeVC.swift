...
class MyRyujumeVC: UIViewController, MyRyujumeProvider {
    ...
    @IBAction func saveBtnAction(_ sender: UIButton) {
        if isInfoFull() {
            putUpdateRyujume(putMyRyujumeIntoParameters())
            let imageData = identityImgView.image?.jpegData(compressionQuality: 1)
            guard let encodedImageDataString = imageData?.base64EncodedString() else { return }
    ...
    func postUpdateIdentityImg(identityImg: String) {
        httpClient
            .post(url: AuthAPI.postRegister.getPath(), params: ["profileImg" : identityImg], header: Header.tokenIsExist.getHeader())
    ...
    func putUpdateRyujume(_ ryujume: Parameters) {
        print(ryujume)
        httpClient
            .put(url: MyPageAPI.putUpdateRyujume.getPath(), params: ryujume, header: Header.tokenIsExist.getHeader())
            .responseJSON(completionHandler: { [weak self] (response) in
                guard let strongSelf = self else { return }
    ...
    func putMyRyujumeIntoParameters() -> Parameters {
        return ["phoneNumber" : phoneNumberTxtField.text!,
